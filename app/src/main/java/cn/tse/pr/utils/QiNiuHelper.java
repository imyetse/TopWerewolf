...
public class QiNiuHelper {
    private final static String BASE_URL = "http://or2jn030t.bkt.clouddn.com/";
    private final static String BUCKET = "tse2y";
    private final static String AK = "81zyiDYZLHD2csQDoEIqqDrzQIyVa_NxYSPxvrK2";
    private final static String SK = "O8waJLa20cO_DZFW4QKxMvse5FeWC6eerSUNkcZ0";
    private static QiNiuHelper helper;
    private UploadManager uploadManager;
    private static String token;

    private QiNiuHelper(String accessKey, SecretKeySpec secretKeySpec) {
        ...
        uploadToken().observeOn(AndroidSchedulers.mainThread())
    ...
    private Observable<String> uploadToken() {
        return Observable.create(new Observable.OnSubscribe<String>() {
            @Override
            public void call(Subscriber<? super String> subscriber) {
                try {
                    subscriber.onNext(uploadToken(BUCKET));
                } catch (Exception e) {
    ...
    private void uploadAfterToken(final String filePath, final UploadListener uplistener) {
        uploadToken().observeOn(AndroidSchedulers.mainThread())
                .subscribe(new SimpleRxSubscriber<String>() {
                    @Override
                    public void onNext(String token) {
                        upload(filePath, token, uplistener);
                    }
                });
    }

    public void toUpload(String filePath, UploadListener uplistener) {
        if (TextUtils.isEmpty(token)) {
            uploadAfterToken(filePath, uplistener);
        } else {
            upload(filePath, token, uplistener);
        }
    }
    ...
    public static QiNiuHelper getHelper() {
        if (helper == null) {
            synchronized (QiNiuHelper.class) {
                byte[] sk = StringUtils.utf8Bytes(SK);
                SecretKeySpec secretKeySpec = new SecretKeySpec(sk, "HmacSHA1");
                helper = new QiNiuHelper(AK, secretKeySpec);
            }
        }
        return helper;
    }

    public void upload(String filePath, String token, final UploadListener uplistener) {
        String expectKey = UUID.randomUUID().toString() + "." + filePath;
        File file = new File(filePath);
        if (!file.exists()) {
            Lg.e("图片地址不存在>>>>" + filePath);
            return;
        }
        uploadManager.put(new File(filePath), expectKey, token, new UpCompletionHandler() {

            @Override
            public void complete(String key, ResponseInfo info, org.json.JSONObject response) {
                if (info.isOK()) {
                    uplistener.onSuccess(BASE_URL + key);
                } else {
                    uplistener.onFailed(info.error);
                }
            }
        }, new UploadOptions(null, "file-type", true, null, null));
    }
